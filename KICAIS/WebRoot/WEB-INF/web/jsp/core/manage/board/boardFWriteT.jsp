		...
		<td>
			<select name='faq_cd'>
				...
				<c:forEach items="${faqcdList}" var="faqcd" varStatus="i">
					<option value=${faqcd.FAQ_CD }>${faqcd.FAQ_NM}</option>
				</c:forEach>
			</select>
		</td>
	</tr>
	<tr>
		<th scope="col" id="odd">제목</td><td colspan='3'><input type='text' name='faq_title' size=100 maxlength='100'></td>
	</tr>
	<tr>
...
<div class="btn">						
	...
	<div class="rightbtn">
		...
		<c:if test="${writeGrant != '0' }">
			<input type="button" class="button02" value="저장" style="margin-right:10px;" onclick="jf_submit()">
		</c:if>	
		<!-- 권한 에 따른 쓰기기능 제어 끝  -->	
		<input type="button" class="button04" value="목록으로" style="margin-right:10px;" onclick="jf_go()">
