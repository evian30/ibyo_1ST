...
var gridTitle_1st_pop 		= "이슈정보";	  				      // 그리드 제목
var	render_1st_pop			= "user-grid";	        			  // 렌더(화면에 그려질) 할 id 
var	pageSize_1st_pop		= 10;	  							  // 그리드 페이지 사이즈	
var	proxyUrl_1st_pop		= "/com/issue/issueInfoPop.sg"; // 결과 값 페이지
...
var userColumns_1st_pop =  [
						   ...
						   ,{header: "이슈구분코드", 	width: 60,  sortable: true, dataIndex: 'ISSUE_TYPE_CODE',editor: new Ext.form.TextField({}) , hidden : true}                                                    
						   ,{header: "이슈구분",   	width: 60,  sortable: true, dataIndex: 'ISSUE_TYPE_NAME',editor: new Ext.form.TextField({})}
						   ,{header: "이슈코드",  	width: 100, sortable: true, dataIndex: 'ISSUE_CODE', 	 editor: new Ext.form.TextField({})}                  
						   ,{header: "순번",			width: 50,  sortable: true, dataIndex: 'ISSUE_INFO_SEQ', editor: new Ext.form.TextField({})}
						   ,{header: "이슈내용",	    width: 80,  sortable: true, dataIndex: 'ISSUE_COMMENT',	 editor: new Ext.form.TextField({})}
						   ,{header: "비고",	  	    width: 50,  sortable: true, dataIndex: 'NOTE', 			 editor: new Ext.form.TextField({}) , hidden : true}
						   ,{header: "사용여부", 		width: 50,  sortable: true, dataIndex: 'USE_YN' , 		 editor: new Ext.form.TextField({}) , hidden : true}  
						   ,{header: "최종변경자ID", 	width: 70, sortable: true, dataIndex: 'FINAL_MOD_ID' ,  editor: new Ext.form.TextField({}) , hidden : true}  
...
function fnSearch(){  	
	...
	Ext.Ajax.request({   
		url: "/com/issue/issueInfoPopList.sg"
		,method : 'POST'   
		...
		,params : {						// 조회조건을 DB로 전송하기위해서 조건값 설정
				  start 	    : start_1st_pop
    			, limit         : limit_1st_pop
    			//, src_issue_comment  : Ext.get('src_issue_comment').getValue()
...
function fnPagingValue_1st_pop(){
	...
	try{
    	...
    	GridClass_1st_pop.store.setBaseParam('src_issue_comment'	  ,Ext.get('src_issue_comment').getValue());
    	GridClass_1st_pop.store.setBaseParam('src_issue_type_code'  ,Ext.get('src_issue_type_code').getValue());
	}catch(e){
...
function fnSelectPopUpGridRow1st(){
	...
	if(records.length == 0){
		...
		window.opener.fnIssuePopValue(records);		//부모창으로 함수호출
		window.close();
	}
}
</script>
						    			...
						    			<td width="50%">
											<div tabindex="-1" class="x-form-item " >
												...
												<div style="padding-left: 30px;" class="x-form-element">
													<select name="src_issue_type_code" id="src_issue_type_code" style="width: 32%;" type="text" class=" x-form-select x-form-field" >
										...
										<td width="50%">
											<div tabindex="-1" class="x-form-item " >
												...
												<div style="padding-left: 0px;" class="x-form-element">
													<input type="text" id="src_issue_comment" name="src_issue_comment" autocomplete="off" size="18" class=" x-form-text x-form-field" style="width: auto;">
												</div>
											</div>
										</td>
