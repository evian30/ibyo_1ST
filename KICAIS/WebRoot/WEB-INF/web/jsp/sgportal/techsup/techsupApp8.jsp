...
<script type="text/javascript" src="/resource/common/js/MultiUpload.js" ></script> 
...
function jf_pjtProdModView(){
	...
	document.vForm2.action = "/sgportal/techsup/techsupApp8.sg";
...
function jf_pjtModForm(){
	if(Validator.validate(document.vForm)){
		oper_code();
		document.vForm.actType.value = 'mod';
		document.vForm.action = "/sgportal/techsup/pjtSupProdAction.sg?ctChk=eight";
		document.vForm.submit();
	}
}
...
<form name='vForm' method='post' enctype="multipart/form-data">   
	...
	<input type="hidden" name="reject_yn" id="reject_yn" value="Y" /> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="grid" rules="cols">
	<tr>
		<th colspan="2" scope="col" id="odd" style="width:15%">* 서버명</th>
		<td colspan="3"> 
			<c:choose>
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">* IP주소</th>
	<td colspan="3">
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">* 제품명</th>
	<td colspan="3"> 
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">수량</th>
	<td colspan="3"> 
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">* OS</th>
	<td style="width:35%">
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
			<select  name="setup_os_no" id="setup_os_no" title="OS" class="validate-required-select">
				<option value="">
					선택
				</option>
	...
	<th scope="col" id="odd" style="width:15%">OS 경로</th>
	<td style="width:35%">
		<c:choose>
			<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
				<input title="OS경로" type='text'  name='setup_path' size=60 maxlength='100' 
...
<tr>
	<th colspan="2" scope="col" id="odd" >SDK</th>
	<td>
		<input name="o_sdk" type="hidden" value="${prodView.SDK_NO}" />
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" >WEB</th>
	<td>
		<input name="o_web" type="hidden" value="${prodView.WEB_NO}" />
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" >WAS</th>
	<td>
		<input name="o_was" type="hidden" value="${prodView.WAS_NO}" />
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	...
	<th scope="col" id="odd" style="width:15%">Cab</th>
	<td colspan="3">
...
<tr>
	<th scope="col" id="odd" style="width:15%">Lib</th>
	<td colspan="3">
...
<tr>
	<th scope="col" id="odd" style="width:15%">Config</th>
	<td colspan="3">
...
<tr>
	<th scope="col" id="odd" style="width:15%">인증서</th>
	<td colspan="3">
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">설치구분</th>
	<td colspan="3">
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">업무프로세스</th>
	<td colspan="3">
		<c:choose>
		<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">로그경로</th>
	<td colspan="3">
...
<tr>
   <th id="odd" scope="col"  colspan="2">첨부파일
   <td colspan="3">
   	  <c:choose>
...
<tr>
	<th colspan="2" scope="col" id="odd" style="width:15%">비고</th>
	<td colspan="3">
		<c:choose>
			<c:when test="${pMap.actType == 'ins' || pMap.actType == 'mod'}">
			...
			<c:otherwise>
			 	<textarea id='pjt_contents' name='prod_use_etc' style='border:1 solid; border-color:#D4D4D4; width:100%; height:110;' readonly="readonly">${prodView.PROD_USE_ETC}</textarea>
			</c:otherwise>
		</c:choose> 
	</td> 
</tr> 
...
<c:choose>	
	...
	<c:when test="${result == 'false'}"> 
			<script> 
				alert("문제가 발생하여 저장하지 못하였습니다."); 
				window.location.href = '/sgportal/techsup/techsupApp8.sg?actType=ins&prod_no=${pMap.prod_no}&pjt_no=${pMap.pjt_no}&prod_seq=${pMap.prod_seq}';
			</script>  
	</c:when>
</c:choose> 