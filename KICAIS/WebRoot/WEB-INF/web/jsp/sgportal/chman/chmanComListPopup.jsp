...
<!-- 
	...
	function jf_search(){
		...
		document.vForm.action = "/sgportal/chman/chmanComList.sg?viewName=/sgportal/chman/chmanComListPopup";
	...
	function jf_regChmanCom(){ 
		...
		document.vForm.action = "/sgportal/chman/chmanComView.sg?viewName=/sgportal/chman/chmanComListPopup";
		document.vForm.submit(); 
	}
	
	function jf_comView(corp_no, corp_nm, corp_sect_code){
		opener.document.vForm.chman_sect_code.value = corp_sect_code;
		opener.document.vForm.corp_no.value = corp_no;
		opener.document.vForm.corp_nm.value = corp_nm;
	...
	<c:forEach items="${companyList}" var="board" varStatus="i">
	    ...
	    <!-- 권한 에 따른 쓰기기능 제어 끝  -->	
	        <td align='left'>
	        	...
	        	<a href="javascript:jf_comView('${board.CORP_NO}','${board.CORP_NM}','${board.CORP_SECT_CODE}')">
	        		${board.CORP_NM}
	        	</a>
	        	</STRIKE>
	        	</c:if>
